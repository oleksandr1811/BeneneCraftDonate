...
class Qiwi {
    ...
    public function __construct($token, $phone, $host) {
        $this->_token = $token;
        $this->_phone = $phone;
        $this->_url = $host /*'https://edge.qiwi.com/'*/;
    }
    ...
    public function getTransaction($transactionId, Array $params = []) {
        return $this->sendRequest('payment-history/v2/transactions/' . $transactionId .'/', $params);
    }

    public function getCheque($transactionId, Array $params = []) {
        return $this->sendRequest('payment-history/v1/transactions/' . $transactionId .'/cheque/file', $params);
    }
    ...
    public function sendMoneyToQiwi(Array $params = []) {
        return $this->sendRequest('sinap/api/v2/terms/99/payments', $params, true);
    }

    public function sendMoneyToProvider($providerId, Array $params = []) {
        return $this->sendRequest('sinap/api/v2/terms/'. $providerId .'/payments', $params, true);
    }

    public function sendMoneyToOther(Array $params = []) {
        return $this->sendRequest('sinap/api/v2/terms/1717/payments', $params, true);
    }
}
?>